...
function renderLayout(title, description, content, activePage, storeMap, availableStoreIDs, deals) {
    ...
    <body class="text-slate-200 antialiased flex flex-col min-h-screen mesh-gradient">
        ...
        <footer class="bg-slate-950/50 pt-10">
            ${seoFooter}
...
async function build() {
    ...
    const browseContent = `
        <div class="max-w-7xl mx-auto px-6">
            <h2 class="text-4xl font-black text-white mb-12 uppercase tracking-tighter">Browse All Deals</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ${data.deals.map(d => generateCard(d, data.storeMap)).join('')}
            </div>
        </div>`;
    fs.writeFileSync(path.join(OUTPUT_DIR, 'browse.html'), renderLayout('Browse Deals', 'Search all PC game sales.', browseContent, 'browse', data.storeMap, availableStoreIDs, data.deals));

    // Free Page
    const freeContent = `
        <div class="max-w-7xl mx-auto px-6">
            <h2 class="text-4xl font-black text-emerald-400 mb-12 uppercase tracking-tighter">100% Free Drops</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ${data.freeDeals.length > 0 ? data.freeDeals.map(d => generateCard(d, data.storeMap, true)).join('') : '<p class="text-slate-500">No free deals right now.</p>'}
            </div>
        </div>`;
    ...
    ['index.html', 'browse.html', 'free.html', 'about.html', 'under-5.html', 'under-10.html'].forEach(p => {
        xml += `  <url><loc>${SITE_DOMAIN}/${p}</loc><lastmod>${today}</lastmod><priority>${p === 'index.html' ? '1.0' : '0.8'}</priority></url>\n`;
    });
    xml += `</urlset>`;
    fs.writeFileSync(path.join(OUTPUT_DIR, 'sitemap.xml'), xml);

    console.log('✅ Success! Full Premium Application Built.');
}
